...
class Q_Learning:
    def __init__(self, env, alpha, gamma, epsilon, numberEpisodes, numberOfBins, lowerBounds, upperBounds, solve_threshold):
        self.env = env
        self.alpha = alpha
        self.gamma = gamma
    ...
    def simulateEpisodes(self):
        for indexEpisode in range(self.numberEpisodes):
            ...
            if indexEpisode >= 100:
                if np.mean(self.sumRewardsEpisode[-100:]) >= self.solve_threshold:
                    print(f"Problem solved in {indexEpisode} episodes!")
                    break
    ...
    def moving_average(self, rewards, window_size):
